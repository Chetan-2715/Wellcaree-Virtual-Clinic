...
import { CheckCircle2, Calendar, User, Stethoscope, Send, ArrowRight, ArrowLeft, Copy, X, Maximize2 } from 'lucide-react';
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <p className="text-gray-600">
              Thank you for choosing Wellcaree. Remember to <strong>send the payment screenshot</strong> in the WhatsApp chat to confirm your appointment.
            </p>
          </div>
          <button
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-center mb-12 space-x-4 md:space-x-8">
          {[
            ...
            { s: 2, icon: Stethoscope, label: "Condition" },
            { s: 3, icon: Calendar, label: "Payment & Confirm" }
          ].map((item, i) => (
        ...
        {/* Form Container */}
        <div className="bg-white p-8 md:p-12 rounded-[3rem] shadow-xl border border-gray-100 relative">
            ...
            {step === 3 && (
              ...
              >
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    <h3 className="text-2xl font-serif font-bold text-clinic-900">Order Summary</h3>
                  ...
                  <div className="space-y-4">
                    <h3 className="text-2xl font-serif font-bold text-clinic-900">Payment Required</h3>
                    <div className="bg-white p-6 rounded-2xl border-2 border-dashed border-clinic-200 flex flex-col items-center text-center space-y-4">

                      {/* QR Code Trigger for Modal */}
                      <div
                        className="bg-gray-100 p-2 rounded-xl cursor-pointer group relative"
                        onClick={() => setIsQRExpanded(true)}
                      >
                        <img src="/images/payment_qr.jpg" alt="Scan to Pay" className="w-48 h-48 object-contain rounded-lg group-hover:opacity-90 transition-opacity" />
                        <div className="absolute inset-0 flex items-center justify-center bg-black/10 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity">
                          <Maximize2 className="text-white drop-shadow-lg" size={32} />
                        </div>
                      </div>

                      <div>
                        <p className="text-sm font-bold text-gray-500 mb-1">UPI ID for Payment</p>
                        <button
                          onClick={() => copyToClipboard('9175930155@okbizaxis')}
                          className="flex items-center space-x-2 bg-gray-100 px-4 py-2 rounded-lg hover:bg-gray-200 transition-colors"
                        >
                          <span className="font-mono text-gray-800">9175930155@okbizaxis</span>
                          <Copy size={16} className="text-gray-500" />
                        </button>
                      </div>
                      <p className="text-xs text-clinic-600 font-bold bg-clinic-50 px-3 py-1 rounded-full">
                        Scan the QR code to pay ₹{price}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-orange-50 border border-orange-100 rounded-xl space-y-2">
                  <h4 className="font-bold text-orange-800 text-sm uppercase tracking-wide">Important Instructions:</h4>
                  <ol className="list-decimal list-inside text-sm text-orange-900/80 space-y-1 ml-2">
                    <li>Scan the QR code above and complete the payment of <strong>₹{price}</strong>.</li>
                    <li>Take a screenshot of the payment success screen.</li>
                    <li>Click the <strong>Proceed to WhatsApp</strong> button below.</li>
                    <li>Send the pre-filled message <strong>AND attach your payment screenshot</strong> in the chat.</li>
                    <li>The <strong>response</strong> will be given only <strong>after the payment is done</strong> and the <strong>payment screenshot is received</strong>.</li>
                  </ol>
                </div>
        ...
        {isQRExpanded && (
          ...
          >
            <motion.div
              ...
              className="bg-white p-4 rounded-3xl relative max-w-lg w-full"
            ...
            >
              ...
              <div className="flex flex-col items-center space-y-4 pt-8 pb-4">
                ...
                <img src="/images/payment_qr.jpg" alt="Payment QR Expanded" className="w-full h-auto rounded-xl" />
                <p className="text-center text-gray-500 font-medium">Use any UPI app to scan and pay</p>
              </div>
            </motion.div>
          </motion.div>
        )}
